...
object Poker {
  ...
  def checkPair(list: List[Card]): Int = {
    val value = List(list.count(_.number == list(0).number),list.count(_.number == list(1).number),list.count(_.number == list(2).number),list.count(_.number == list(3).number),list.count(_.number == list(4).number))
